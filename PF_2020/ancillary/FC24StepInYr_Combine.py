...
#upports both FC2016 and FC2024 headers: FC2016 date from Y/M/D; FC2024 date from Year + Day of year (day); renames C mass of forest litter and deadwood to C mass of forest debris; does a QC numeric pass
#%%##
import os
import glob
import pandas as pd
...
for f in filenames:
    try:
        ...
        if (date_series != '').any():
            ...
            date_col = df['Date']
            blank_mask = date_col.isna() | date_col.eq('')
            if blank_mask.any():
                next_dates = date_col.shift(-1)
                years = pd.to_datetime(next_dates, dayfirst=True, errors='coerce').dt.year
                fill_str = pd.Series([''] * len(df), index=df.index)
                valid_years = years.notna()
                fill_str.loc[valid_years] = '01/01/' + years[valid_years].astype(int).astype(str)
                df.loc[blank_mask, 'Date'] = fill_str[blank_mask].values
